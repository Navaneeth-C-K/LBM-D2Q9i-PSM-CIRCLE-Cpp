...
//---------cricle stuff complete------------------------------------
    ...
    ofstream outRecir("Recirculation_circle.csv");

//------------while loop starts HERE!!---------------------------------------

    //while(error > 1e-6) //this while loop is used for convergence of velocity components!
    while(t< total_time_lu)
    {
       ...
       for(int i = nodes_x+3; i <= total_nodes-(nodes_x+4); i++) //colission step
        ...
        for(int i = nodes_x+3; i <= total_nodes-(nodes_x+4); i++)
        {
            ...
            // created a skip for all the corners!
            if(i == top_left || i == top_right || i == bot_left || i == bot_right) { continue; }
           ...
           //explanation for variable k given on top!
            else if((i-1)%(pad_x) == 0)//left wall
            {
                ...
                mesh[i].f[1] = mesh[i].f[3] + ((2.0/3.0) * mesh[i].rho * u_x);
        ...
        outDrag << t << "\t" << ((drag1 + drag2 + drag3 + drag4)/(0.5 * ( U_star) * ( U_star) * d)) << "\n";

        outLift << t << "\t" << ((lift1 + lift2 + lift3 + lift4)/(0.5 * (U_star) * (U_star) * d)) << "\n";
...
//--------------Post while loop things!!--------------------------------------------
    ...
    int cylinder_end_x = ((50 * d) / 4) + d; 

    // 2. Find the 1D mesh index for the centerline, exactly 1 node behind the cylinder
    int wake_start_idx = (gad * pad_x) + cylinder_end_x + 1;
    ...
    cout << "Breuer et al. comparision value : " << length / d << endl;
